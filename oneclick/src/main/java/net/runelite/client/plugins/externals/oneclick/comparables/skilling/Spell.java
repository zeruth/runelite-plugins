...
public class Spell extends ClickCompare
{
	...
	{
		this.spell = spell;
		this.cast = (event) -> event.getOpcode() == MenuAction.WIDGET_TYPE_2.getId() &&
			event.getOption().equals("Cast") &&
			event.getTarget().contains("<col=00ff00>" + spell + "</col><col=ffffff> -> ");
		this.reset = (event) -> event.getOpcode() == MenuAction.RUNELITE.getId() &&
			event.getIdentifier() == -1;
		this.set = (event) -> event.getOpcode() == MenuAction.RUNELITE.getId();
	}
	...
	public void modifyClick(MenuEntry event)
	{
		...
		if (cast.test(event))
		{
			...
			if (pair.getLeft() != -1)
			{
				event.setOpcode(MenuAction.ITEM_USE_ON_WIDGET.getId());
